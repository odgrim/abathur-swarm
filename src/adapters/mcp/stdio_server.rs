    ...
    G: GoalRepository + Send + Sync + 'static,
{
    ...
    fn always_available_tools() -> Vec<serde_json::Value> {
        vec![
            ...
            serde_json::json!({"name":"workflow_fan_out","description":"Split the current workflow phase into parallel subtasks. Each slice gets its own subtask. Specify `agent` per slice to assign the specialist inline (preferred) — this avoids a race where the scheduler grabs the subtask before a separate task_assign call.","inputSchema":{"type":"object","properties":{"task_id":{"type":"string","description":"UUID of the parent task to fan out"},"slices":{"type":"array","items":{"type":"object","properties":{"description":{"type":"string","description":"Description of this slice's work"},"agent":{"type":"string","description":"Agent template name to assign to this subtask (recommended — avoids race with scheduler)"}},"required":["description"]},"description":"Array of work slices — one subtask is created per slice"}},"required":["task_id","slices"]}}),
