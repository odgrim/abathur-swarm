    ...
    G: GoalRepository + Send + Sync + 'static,
{
    ...
    fn always_available_tools() -> Vec<serde_json::Value> {
        vec![
            ...
            serde_json::json!({"name":"workflow_fan_out","description":"Split the current workflow phase into parallel subtasks. Each slice gets its own subtask. You MUST set `agent` on every slice — create or reuse an agent template first (call agent_list), then pass its name here. Fan-out will be rejected if any slice is missing an agent.","inputSchema":{"type":"object","properties":{"task_id":{"type":"string","description":"UUID of the parent task to fan out"},"slices":{"type":"array","items":{"type":"object","properties":{"description":{"type":"string","description":"Description of this slice's work"},"agent":{"type":"string","description":"Agent template name to assign to this subtask (REQUIRED — fan-out fails without this)"}},"required":["description","agent"]},"description":"Array of work slices — one subtask is created per slice"}},"required":["task_id","slices"]}}),
