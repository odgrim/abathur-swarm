...
impl Default for BatchInsertResult {
    fn default() -> Self {
        Self::new()
    }
}

/// Generate a consistent idempotency key for a task.
///
/// This function MUST be used by all code paths that auto-generate idempotency keys
/// to ensure consistency. Using different algorithms in different places can cause
/// duplicate task creation bugs.
///
...
///
/// # Arguments
/// * `task` - The task to generate an idempotency key for
///
/// # Returns
/// A deterministic idempotency key in the format `auto:{agent_type}:{parent_id}:{hash}`
pub fn generate_auto_idempotency_key(task: &Task) -> String {
