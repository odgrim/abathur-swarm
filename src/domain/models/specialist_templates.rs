...
- **Trace callers, not just callees**: The issue example often uses one specific function, but the fix may need to cover other public APIs that share the same internal code path or have parallel implementations. Grep for all callers of the affected internal function and for sibling/wrapper methods that provide similar functionality. Report the full set of public entry points that need to handle the fix, not just the one in the issue example.
