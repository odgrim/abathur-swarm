...
impl<T: TrajectoryRepository, M: MemoryRepository, O: OverseerMeasurer> ConvergenceEngine<T, M, O> {
    ...
    pub async fn converge_parallel(
        ...
        for _ in 0..n {
            ...
            if !eligible.is_empty() {
                let strategy = bandit.select(&attractor.classification, &eligible, &sample.policy);
                let (artifact, tokens_used, _wall_time_ms) =
                    self.execute_strategy(&strategy, &mut sample).await?;
                let mut observation = self.measure_artifact(&artifact, &strategy, &sample).await?;
                observation.tokens_used = tokens_used;
                let control = self
                    .iterate_once(&mut sample, &mut bandit, &strategy, observation)
                    .await?;

                if matches!(control, LoopControl::OverseerConverged) {
                    let final_seq = sample.observations.last().map(|o| o.sequence).unwrap_or(0);
                    let outcome = ConvergenceOutcome::Converged {
        ...
        loop {
            ...
            if all_exhausted {
                ...
                self.finalize(&mut trajectories[best_idx], &outcome, &bandits[best_idx])
                    .await?;
                return Ok(outcome);
            }
            ...
            let selected_idx = (0..n)
                .filter(|&i| active[i] && trajectories[i].budget.has_remaining())
                .max_by(|&a, &b| {
            ...
            let strategy = bandit.select(&attractor.classification, &eligible, &traj.policy);
            let (artifact, tokens_used, _wall_time_ms) =
                self.execute_strategy(&strategy, traj).await?;
            let mut observation = self.measure_artifact(&artifact, &strategy, traj).await?;
            observation.tokens_used = tokens_used;
            let control = self
            ...
            if matches!(control, LoopControl::OverseerConverged) {
                let final_seq = traj.observations.last().map(|o| o.sequence).unwrap_or(0);
                let outcome = ConvergenceOutcome::Converged {
