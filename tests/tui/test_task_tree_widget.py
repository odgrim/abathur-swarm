...
@pytest.fixture
def dependency_graph(sample_tasks: list[Task]) -> dict[UUID, list[UUID]]:
    """Create dependency graph for sample tasks.
...
def test_load_tasks(sample_tasks: list[Task], dependency_graph: dict[UUID, list[UUID]]):
    """Test loading tasks into the widget."""
    widget = TaskTreeWidget("Tasks")
    widget.load_tasks(sample_tasks, dependency_graph)
...
async def test_keyboard_navigation_down(
    sample_tasks: list[Task], dependency_graph: dict[UUID, list[UUID]]
):
    """Test keyboard navigation down with j/down arrow."""
    app = TestApp(sample_tasks, dependency_graph)
    ...
    async with app.run_test() as pilot:
        ...
        assert tree.selected_task_id == sample_tasks[0].id

        # Check message was emitted
        assert len(app.messages_received) == 1
        assert app.messages_received[0].task_id == sample_tasks[0].id
...
async def test_node_expanded_message_emission(
    ...
    app = TestAppWithNodeExpanded(sample_tasks, dependency_graph)
...
async def test_load_tasks_clears_existing_state(
    ...
    widget.load_tasks(sample_tasks[:1], dependency_graph)
